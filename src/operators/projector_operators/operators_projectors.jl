...
"""
mutable struct SPMSProjectorOperator{
        SPMSB_IN   <: SPBasis{SPMSBS_IN} where {SPMSBS_IN<:Union{SPMSBasisState{BS} where BS, SPMSCompositeBasisState{B} where B, DelocalizedBasisState{BS} where BS, TetramerBasisState{BS} where BS, TdSymBasisState}},
        SPMSB_OUT  <: SPBasis{SPMSBS_OUT} where {SPMSBS_OUT<:Union{SPMSBasisState{BS} where BS, SPMSCompositeBasisState{B} where B, DelocalizedBasisState{BS} where BS, TetramerBasisState{BS} where BS, TdSymBasisState}},
        SPO <: AbstractSPMSOperator{SPMSB_IN}
    } <: AbstractSPMSOperator{SPMSB_OUT}
